...
test("[Discovery] 구매 흐름은 장바구니 직행보다 상품 목록 진입점을 우선한다", async () => {
  const fixtureRoot = join(tmpdir(), `wedge-runner-discovery-product-list-${process.pid}-${Date.now()}`);
  await mkdir(fixtureRoot, { recursive: true });

  try {
    const fixturePath = join(fixtureRoot, "index.html");
    await writeFile(fixturePath, createProductListPreferredCheckoutFixtureHtml(), "utf8");
    const fixtureUrl = pathToFileURL(fixturePath).toString();
    ...
    assert.ok(checkoutRecommendation?.suggested_target);
    assert.equal(checkoutRecommendation.suggested_target.selector, "#product-list");
    assert.equal(checkoutRecommendation.suggested_target.href_contains, "/goods/goods_list.php?cateCd=001");
    assert.ok(!checkoutRecommendation.suggested_target.href_contains?.includes("/order/cart"));
    assert.ok(checkoutRecommendation.evidence_summary?.matched_signals.some((signal) =>
      signal.signal_type === "purchase_checkout_product_browse_url"
    ));
  } finally {
    await rm(fixtureRoot, { recursive: true, force: true });
  }
...
test("[Discovery] 구매 흐름은 상품 종류 단어 없이 카드/가격 패턴으로 상품을 인지한다", async () => {
  const fixtureRoot = join(tmpdir(), `wedge-runner-discovery-product-card-${process.pid}-${Date.now()}`);
  await mkdir(fixtureRoot, { recursive: true });

  try {
    const fixturePath = join(fixtureRoot, "index.html");
    await writeFile(fixturePath, createGenericProductCardCheckoutFixtureHtml(), "utf8");
    const fixtureUrl = pathToFileURL(fixturePath).toString();
    ...
    assert.ok(checkoutRecommendation?.suggested_target);
    assert.equal(checkoutRecommendation.suggested_target.selector, "a[href=\"/detail/sku-100\"]");
    assert.equal(checkoutRecommendation.suggested_target.href_contains, "/detail/sku-100");
    assert.ok(!checkoutRecommendation.suggested_target.href_contains?.includes("/cart"));
  } finally {
    await rm(fixtureRoot, { recursive: true, force: true });
  }
});

test("[Discovery] 이미지 alt 링크는 부모 링크를 추천 진입점으로 고정한다", async () => {
  const fixtureRoot = join(tmpdir(), `wedge-runner-discovery-alt-link-${process.pid}-${Date.now()}`);
  await mkdir(fixtureRoot, { recursive: true });

  try {
    const fixturePath = join(fixtureRoot, "index.html");
    await writeFile(fixturePath, createImageAltLinkDiscoveryFixtureHtml(), "utf8");
    const fixtureUrl = pathToFileURL(fixturePath).toString();
    ...
    assert.ok(contactRecommendation?.suggested_target);
    assert.equal(contactRecommendation.suggested_target.text, "1:1문의게시판");
    assert.equal(contactRecommendation.suggested_target.selector, "#inquiry-board");
    assert.equal(contactRecommendation.suggested_target.href_contains, "/login?next=/inquiry");
    assert.ok(contactRecommendation.evidence_summary?.matched_signals.some((signal) =>
      signal.source === "alt" && signal.value === "1:1문의게시판"
    ));
    assert.ok(contactRecommendation.evidence_summary?.matched_signals.some((signal) =>
      signal.source === "selector" && signal.signal_type === "contact_linked_image_parent"
    ));

    const observation = (result.checkpoints.at(0)?.observations ?? []).find(
      (nextObservation) => nextObservation.type === "contact_candidate"
    );
    assert.equal(observation?.data?.selector, "#inquiry-board");
    assert.equal(observation?.data?.source_tag_name, "img");
    assert.equal(observation?.data?.linked_image_alt, true);
  } finally {
    await rm(fixtureRoot, { recursive: true, force: true });
  }
});
...
test("[Discovery] shallow navigation lowers login-required contact candidates below public alternatives", async () => {
  const requestedPaths = new Map<string, number>();
  const server = await startFixtureServer(requestedPaths, {
    rootHtml: `<!doctype html>
<html lang="ko">
  <head><title>Contact fixture</title></head>
  <body>
    <main>
      <a id="qa-login" href="/qa-login"><img alt="1:1문의게시판" src="/qa.png" /></a>
      <a id="question" href="/question"><img alt="고객문의 게시판" src="/question.png" /></a>
    </main>
  </body>
</html>`
  });
  ...
  try {
    ...
    assert.ok(contactRecommendation?.suggested_target);
    assert.equal(contactRecommendation.suggested_target.selector, "#question");
    assert.equal(contactRecommendation.suggested_target.href_contains, "/question");
    assert.ok(!contactRecommendation.suggested_target.href_contains?.includes("login"));
    assert.ok(contactRecommendation.evidence_summary?.matched_signals.some((signal) =>
      signal.signal_type === "contact_login_required" && signal.value.includes("/member/login.php")
    ));
  } finally {
    await closeServer(server);
  }
...
function createProductListPreferredCheckoutFixtureHtml(): string {
  return `<!doctype html>
<html lang="ko">
  <head><title>Product list checkout fixture</title></head>
  <body>
    <main>
      <a id="cart-link" href="/order/cart.php">장바구니</a>
      <a id="product-list" href="/goods/goods_list.php?cateCd=001">상품 목록</a>
      <a id="product-detail" href="/goods/goods_view.php?goodsNo=1088">상품 상세 보기</a>
    </main>
  </body>
</html>`;
}

function createGenericProductCardCheckoutFixtureHtml(): string {
  return `<!doctype html>
<html lang="ko">
  <head><title>Generic product card fixture</title></head>
  <body>
    <main>
      <a id="cart-link" href="/cart">Cart</a>
      <section class="grid">
        <a class="product-card" href="/detail/sku-100">
          <img src="/a.png" alt="Model A" />
          <strong>Model A</strong>
          <span class="price">29,000원</span>
          <span class="option">Option required</span>
        </a>
      </section>
    </main>
  </body>
...
function createImageAltLinkDiscoveryFixtureHtml(): string {
  return `<!doctype html>
<html lang="ko">
  <head><title>Image alt linked contact fixture</title></head>
  <body>
    <main>
      <a id="inquiry-board" href="/login?next=/inquiry">
        <img alt="1:1문의게시판" src="inquiry.png" />
      </a>
    </main>
  </body>
</html>`;
...
async function startFixtureServer(
  ...
  const server = createServer((request, response) => {
    ...
    if (path === "/qa-login") {
      response.statusCode = 302;
      response.setHeader("location", "/member/login.php?returnUrl=%2Fqa-login");
      response.end();
      return;
    }
...
<html lang="ko">
  ...
  <body><main><h1>로그인</h1><form><input name="loginId" /><input type="password" /></form></main></body>
</html>`);
      return;
    }
    ...
    if (path === "/question") {
      response.end(`<!doctype html>
<html lang="ko">
  <head><title>고객문의 게시판</title></head>
  <body><main><h1>고객문의 게시판</h1><a href="/question/write">문의 작성</a></main></body>
</html>`);
      return;
    }
