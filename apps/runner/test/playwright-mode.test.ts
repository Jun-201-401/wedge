...
test("[MVP 가입폼] 폼까지 이동해 synthetic 입력/플랜 선택을 수행하고 제출 전 중단한다", async () => {
  ...
  try {
    ...
    const submitReadyCheckpoint = findCheckpoint(result.checkpoints, "signup_008_submit_ready_checkpoint");
    const interactiveComponents = findObservation(submitReadyCheckpoint, "interactive_components");
    const components = readRecordArray(interactiveComponents, "components");
    const emailComponent = components.find((component) => component.selector === "#work-email");
    const companyComponent = components.find((component) => component.selector === "#company");
    const teamSmallComponent = components.find((component) => component.selector === "#team-small");
    const formFields = submitReadyCheckpoint.observations.filter((observation) => observation.type === "form_field");
    const emailField = formFields.find((observation) => observation.field_key === "email");
    const companyField = formFields.find((observation) => observation.field_key === "company");
    const teamSmallField = formFields.find((observation) => observation.field_key === "team-size");

    assert.equal(emailComponent?.describedby_text, "회사 이메일 형식으로 입력하세요.");
    assert.equal(emailComponent?.help_text, "회사 이메일 형식으로 입력하세요.");
    assert.equal(emailComponent?.input_format_hint, "회사 이메일 형식으로 입력하세요.");
    assert.equal(emailComponent?.pattern, ".+@.+\\\\..+");
    assert.equal(emailComponent?.maxlength, 80);
    assert.equal(emailComponent?.visible_required_marker, "*");
    assert.equal(companyComponent?.visible_optional_marker, "optional");
    assert.equal(teamSmallComponent?.group_level_required_state, "mixed");
    assert.equal(emailField?.describedby_text, "회사 이메일 형식으로 입력하세요.");
    assert.equal(emailField?.input_format_hint, "회사 이메일 형식으로 입력하세요.");
    assert.equal(emailField?.input_type, "email");
    assert.equal(emailField?.required, true);
    assert.equal(emailField?.visible_required_marker, "*");
    assert.equal(companyField?.visible_optional_marker, "optional");
    assert.equal(teamSmallField?.group_level_required_state, "mixed");
  } finally {
    await rm(fixtureRoot, { recursive: true, force: true });
    await rm(artifactsRoot, { recursive: true, force: true });
  }
...
test("[MVP 가격/결제] checkout 진입점까지 이동하되 실제 결제 commit 전 중단한다", async () => {
  ...
  try {
    ...
    const finalSubmitRelation = checkoutContextValue.final_submit_relation as Record<string, unknown>;
    ...
    assert.equal(accordions[0]?.panel_relationship, "details_summary");
    ...
    assert.equal(finalSubmitRelation.related, true);
    assert.equal(finalSubmitRelation.relation_type, "summary_before_submit");
  } finally {
    await rm(fixtureRoot, { recursive: true, force: true });
    await rm(artifactsRoot, { recursive: true, force: true });
  }
...
test("[Settle] response/item_count_change 조건 불만족은 timeout 결과로 보고한다", async () => {
  ...
  try {
    ...
    const currentCount = itemCountTimeoutResult.details?.currentCount as number;
    assert.ok(currentCount >= 1);
    assert.ok(currentCount < 4);
    ...
    <main>
      ...
      <section id="signup-form" style="margin-top: 960px;">
        ...
        <form>
          ...
          <p id="email-help">회사 이메일 형식으로 입력하세요.</p>
          <input id="work-email" name="email" type="email" placeholder="Work email" aria-describedby="email-help" pattern=".+@.+\\\\..+" maxlength="80" required />

          <label for="company">Company <span>(optional)</span></label>
          <input id="company" name="company" type="text" placeholder="Company" />

          <fieldset>
            <legend>Team size required</legend>
            <label><input id="team-small" name="team-size" type="radio" value="small" required /> Small team</label>
            <label><input id="team-large" name="team-size" type="radio" value="large" /> Large team</label>
          </fieldset>
      ...
      <section id="pricing" style="margin-top: 960px;">
        ...
        <article class="plan-card">
          <h3>Starter</h3>
          <p>$19 / month</p>
          <a id="starter-more" href="#starter-details">Learn more</a>
          <a id="starter-plan" href="./checkout.html">Choose Starter</a>
        </article>
        <article class="plan-card">
          <h3>Pro</h3>
          <p>$49 / month</p>
          <a id="pro-more" href="#pro-details">Learn more</a>
        </article>
      </section>
    </main>
    <script>
    ...
    <main>
      ...
      <details id="order-details">
        ...
        <div id="order-details-panel">
          ...
          <p>Required billing terms must be reviewed before payment.</p>
