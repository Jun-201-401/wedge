...
test('getSafeAuthRedirectPath accepts same-origin app paths only', () => {
  ...
  assert.equal(getSafeAuthRedirectPath('', origin), '/');
...
test('getSafeAuthRedirectPath rejects external and backslash redirects', () => {
  ...
  assert.equal(getSafeAuthRedirectPath('?next=https://evil.example/runs/111', origin), '/');
  assert.equal(getSafeAuthRedirectPath('?next=//evil.example/runs/111', origin), '/');
  assert.equal(getSafeAuthRedirectPath('?next=%2F%5Cevil.example', origin), '/');
  assert.equal(getSafeAuthRedirectPath('?next=/\\evil.example', origin), '/');
});
