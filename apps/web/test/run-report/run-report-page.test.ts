...
test('run report page follows the report.html result-first layout', () => {
  ...
  assert.match(source, /aria-pressed=\{selectedFindingId === finding\.id\}/);
  ...
  assert.match(source, /aria-pressed=\{Boolean\(selectedRecommendationFindingId && selectedFindingId === selectedRecommendationFindingId\)\}/);
  assert.match(source, /setSelectedFindingId\(selectedRecommendationFindingId\)/);
  ...
  assert.doesNotMatch(source, /run-report-reason-card--/);
...
test('run report css keeps result-first content in the live simulation cockpit tone', () => {
  ...
  assert.match(css, /\.run-report-validation-check\s*\{[\s\S]*?background: #f8fafc/);
  assert.match(css, /\.run-report-validation-check span\s*\{[\s\S]*?color: #64748b/);
  ...
  assert.doesNotMatch(css.match(/\.run-report-top-finding-card__button\s*\{[^}]*\}/)?.[0] ?? '', /border-left/);
  assert.doesNotMatch(css, /\.run-report-top-finding-card--(?:high|medium) \.run-report-top-finding-card__button\s*\{[\s\S]*?border-left-color/);
  ...
  assert.match(css, /\.run-report-top-finding-card__button\[aria-pressed="true"\]\s*\{[\s\S]*?border-color: #bae6fd/);
