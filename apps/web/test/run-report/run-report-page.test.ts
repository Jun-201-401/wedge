...
test('run report page follows the report.html result-first layout', () => {
  ...
  assert.match(source, /Top Priority Findings[\s\S]*Recommended Nudge[\s\S]*Decision Map[\s\S]*Evidence Details/);
  assert.match(source, /selectedFindingId/);
  assert.match(source, /setSelectedFindingId\(finding\.id\)/);
  assert.match(source, /selectedFinding\?\.previewImageUrl \?\? report\.evidencePreviewUrl/);
  ...
  assert.match(source, /aria-pressed=\{selectedFinding\?\.id === finding\.id\}/);
...
test('run report css keeps result-first content in the live simulation cockpit tone', () => {
  ...
  assert.match(css, /\.run-report-main-column > \.run-report-section--top-findings\s*\{[\s\S]*?background: linear-gradient/);
