...
test('run report css keeps result-first content in the live simulation cockpit tone', () => {
  ...
  assert.match(css, /\.run-report-hero\s*\{[\s\S]*?min-height: 3\.75rem/);
  ...
  assert.match(css, /\.run-report-hero-stats\s*\{[\s\S]*?flex-wrap: wrap/);
  ...
  assert.match(css, /\.run-report-hero-stats div\s*\{[\s\S]*?display: grid/);
  assert.match(css, /\.run-report-hero-stats dt\s*\{[\s\S]*?grid-row: 2/);
  assert.match(css, /\.run-report-hero-stats dd\s*\{[\s\S]*?font-size: 0\.96rem/);
