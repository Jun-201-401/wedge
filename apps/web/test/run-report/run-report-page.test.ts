...
test('run report css keeps result-first content in the live simulation cockpit tone', () => {
  ...
  assert.match(css, /\.run-report-top-finding-list\s*\{[\s\S]*?grid-template-columns: repeat\(3, minmax\(0, 1fr\)\)/);
  assert.match(css, /\.run-report-top-finding-card__button\s*\{[\s\S]*?min-height: 15rem/);
