...
test('run report css keeps result-first content in the live simulation cockpit tone', () => {
  ...
  assert.match(css, /\.run-report-reference-summary__chevron\s*\{[\s\S]*?transition: transform 400ms cubic-bezier\(0\.4, 0, 0\.2, 1\)/);
  ...
  assert.match(css, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.run-report-all-recommendations__panel,[\s\S]*?\.run-report-reference-summary__panel\s*\{[\s\S]*?transition: none/);
