...
test('run monitor css follows the live cockpit visual language', () => {
  ...
  assert.match(css, /\.run-monitor-panel-heading h2,[\s\S]*?\.run-monitor-log h2\s*\{[\s\S]*?color: #475569/);
  ...
  assert.match(css, /\.run-monitor-log__item\s*\{[\s\S]*?transition: background 160ms ease, box-shadow 160ms ease, transform 160ms ease/);
  assert.match(css, /\.run-monitor-log__item:hover\s*\{[\s\S]*?background: rgba\(248, 250, 252, 0\.82\)/);
  assert.match(css, /\.run-monitor-log__item:hover\s*\{[\s\S]*?transform: translateY\(-1px\)/);
  assert.doesNotMatch(css, /\.run-monitor-log__item:hover\s*\{[^}]*border-color:/);
  ...
  assert.match(css, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.run-monitor-log__item\s*\{[\s\S]*?transition: none/);
  assert.match(css, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.run-monitor-log__item:hover\s*\{[\s\S]*?transform: none/);
});
