...
## 목적
이 문서는 Wedge 저장소에서 공통으로 따를 Git 협업 규칙을 정의한다. 목표는 히스토리를 읽기 쉽게 유지하고, PR 리뷰와 롤백을 쉽게 만드는 것이다.
...
## 커밋 메시지
이 저장소는 `Conventional Commits` 제목과 `Lore-style trailer`를 같이 사용한다. 제목 prefix는 영어(`feat:`, `fix:`)로 유지하고, 제목 내용과 본문은 한글로 작성한다. 한 커밋에는 한 가지 의도만 담는다.
...
제목 아래에는 Lore-style trailer를 붙여 의사결정과 검증을 남긴다.
