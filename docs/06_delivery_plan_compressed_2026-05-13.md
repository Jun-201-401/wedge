...
| 구분 | 기간 | 운영 메모 |
|---|---:|---|
| 전체 기간 | 2026-04-13 ~ 2026-05-13 | Sprint 0 + 압축 구현/QA/Final |
| Sprint 0 / 기획 | 2026-04-13 ~ 2026-04-17 | 기획/설계 완료, Jira seed 준비 |
| 구현 시작 | 2026-04-20 | Sprint 1 착수 |
| 압축 구현/QA | 2026-04-20 ~ 2026-05-12 | 기존 구현/안정화 업무를 병합 수행 |
| Final | 2026-05-13 | 최종 릴리즈/시연 |
...
> 2026-05-25 부처님오신날 대체공휴일은 변경된 종료일 이후라 운영 캘린더에서 제외합니다.
...
| Sprint 1 | 4/20–4/24 | 프로젝트 기반 구축 + 첫 브라우저 실행/Evidence | Spring/React/Runner/FastAPI/DB/MQ/S3 골격, Landing CTA checkpoint 저장 |
| Sprint 2 | 4/27–5/1 | Rule Engine/Report 1차 + 핵심 시나리오/Share 착수 | EvidencePacket → JudgeResult → Report UI, Pricing/Share 흐름 1차 |
| Sprint 3 | 5/4–5/8 | Calibration과 RC 진입 | benchmark labeling, false positive 조정, P0 triage/critical fix |
| Sprint 4 / Final | 5/11–5/13 | RC 최종 안정화와 최종 릴리즈/시연 | release build, demo script, sample report, release note 고정 |
...
> 아래 backlog의 기존 Sprint 번호는 기능 scope 묶음입니다. 실제 수행일은 5장과 7장의 2026-04-20 ~ 2026-05-13 압축 일정 기준으로 재배치했습니다.

| 압축 구간 | 기간 | 병합되는 기존 범위 | 주요 산출물 |
|---|---|---|---|
| Sprint 1 | 4/20–4/24 | 기존 Sprint 1 + Sprint 2 핵심 | 프로젝트 골격, Runner/Evidence E2E 1차 |
| Sprint 2 | 4/27–5/1 | 기존 Sprint 3 + Sprint 4 핵심 | Rule/Report 연결, Pricing/Share 1차 |
| Sprint 3 | 5/4–5/8 | 기존 Sprint 4 마감 + Sprint 5 + RC 진입 | Calibration, known issue, P0 triage/fix |
| Sprint 4 / Final | 5/11–5/13 | 기존 Sprint 6 + Final | RC deploy, release build, 최종 시연 |

### Site Discovery / Preflight 일정 오버레이

`rewritedocs.txt` 기준 변경으로 압축 Sprint 1~3에는 URL-first Discovery와 Scenario Recommendation 작업이 포함된다. 기존 Run/Evidence/Judge 흐름을 폐기하지 않고, 정식 Run 앞에 lightweight Preflight를 추가한다.

| Sprint | 추가/조정 범위 |
|---|---|
| Sprint 1 | URL-first create flow UI skeleton, Discovery result mock UI, Discovery API skeleton, `site_discovery` / `scenario_recommendation` DB 초안, `discovery.execute.request` MQ contract 초안, Runner discovery executor 1차 |
| Sprint 2 | first-view checkpoint collection, CTA/form/pricing/contact candidate extractor 1차, `POST /api/discoveries`, `GET /api/discoveries/{discoveryId}`, scenario recommendation mock 또는 rule-based 1차, ScenarioRecommendation UI 연결 |
| Sprint 3 | 실제 recommendation 생성, Run 생성 시 `sourceDiscoveryId` 연결, `scenarioFitStatus` field 표시, full run 안의 scenario fit check, mismatch report/low-fit warning UI, recommendation threshold 조정 |
| Sprint 4 / Final | mismatch benchmark case 검증, recommendation false positive/false negative 검토, demo sample과 release note에 반영 |
...
## 7. 일별 Jira 실행 계획
> 압축 일정 기준: Sprint 0은 유지하고, 구현/QA/Final 업무는 2026-04-20부터 2026-05-13까지 재배치했습니다. 담당자, 업무 성격, 완료 기준은 유지하되 기존 2개 내외 업무일을 1개 업무일에 병합했습니다.
> 2026-05-01 노동절과 2026-05-05 어린이날은 휴무/버퍼로 유지했습니다. 기존 2026-05-25 휴무 항목은 종료일 이후라 RC 진입 전 이슈 정리 업무로 흡수했습니다.
...
**Day Goal:** 프로젝트 기반 구축 착수: Spring/React/Runner/FastAPI/DB/MQ/S3 최소 실행 골격
...
**Day Goal:** Run/API/UI skeleton과 Scenario/Evidence contract 동시 구축
...
**Day Goal:** Runner callback/container/CI와 Sprint 1 통합 리뷰 병합
...
**Day Goal:** 브라우저 실행·checkpoint 저장 1차 구현
...
**Day Goal:** Landing CTA E2E와 Evidence 저장 검증
...
**Day Goal:** 분석 큐/RuleRegistry/Report skeleton 연결
...
**Day Goal:** 첫 Judge Report와 WebSocket/Top Issues 연결
...
| WDAY-083 | 박성환 | WEDGE-EPIC-01 | spring-api | P0 | Report Summary API 구현 | Report Summary UI가 호출 가능한 API 존재 | WDAY-077 |
...
**Day Goal:** Pricing/Share/Permission 설계·구현 착수
...
| WDAY-095 | 박성환 | WEDGE-EPIC-01 | spring-api | P0 | Report Detail API 초안 | Report detail response DTO가 정의됨 | WDAY-083 |
...
| WDAY-098 | 차지훈 | WEDGE-EPIC-02 | runner | P0 | Pricing scenario 실행 + invalid form submit action | pricing/form scenario가 최소 1개 사이트에서 실행됨 | WDAY-092,WDAY-096 |
...
**Day Goal:** Decision Map/Nudge/Share와 Runner 안정성 통합
...
**Day Goal:** Feature Complete Beta 판단과 Calibration 착수
...
**Day Goal:** Benchmark/Idempotency/Soft delete/Rule false positive 분석
...
**Day Goal:** Threshold 조정, RC backlog 구성, known issue 정리
...
**Day Goal:** RC triage와 critical fix, 문서/sample report 정리

| Jira Temp Key | 담당 | Epic | Component | Priority | 그날 할 일 | 완료 기준 | 의존성 |
|---|---|---|---|---|---|---|---|
| WDAY-151 | 박성환 | WEDGE-EPIC-08 | pm | P2 | RC 진입 전 기존 이슈 정리 | 5/11 오전에 P0 bug triage를 시작할 수 있도록 기존 이슈만 정리 |  |
...
### 2026-05-11 (월) — Sprint 4 / Final
**Day Goal:** RC deploy, demo scenario 고정, MCP/OAuth 최소 검증
...
### 2026-05-12 (화) — Sprint 4 / Final
**Day Goal:** Final build와 demo/script/sample report 최종화
...
### 2026-05-13 (수) — Sprint 4 / Final
**Day Goal:** 최종 릴리즈/시연 및 긴급 대응
