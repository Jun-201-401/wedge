...
Runner는 DB에 직접 쓰지 않는다. 제품 기본 Run 시작은 별도 `AgentTask` / `AgentTrace` / `agent.execute.request` 경로로 동작한다. 고정 `ScenarioPlan` replay가 필요한 scripted 경로에서만 Runner가 fixed `ScenarioPlan` executor로 동작한다. ScenarioAuthoring은 별도 `scenario-authoring.execute.request` worker에서 candidate를 생성할 수 있지만, scripted 실행 중 authoring job/result를 실행 DSL처럼 재해석하지 않는다.
...
- ScenarioAuthoring은 V1 계약 경계다. MVP 이후 provider/runtime은 `INTERNAL_LLM`(GMS) 우선, `RULE_BASED` fallback으로 확장한다.
- ScenarioAuthoring은 비동기 계약 경계다. Runner는 별도 `scenario-authoring.execute.request` worker에서 authoring input을 받아 검증 가능한 `ScenarioPlan` candidate를 생성할 수 있다.
- Scripted Run 경로에서 Runner는 Spring이 materialize한 고정 `ScenarioPlan`만 실행하며 authoring job/result를 실행 DSL처럼 재해석하지 않는다.
