...
Runner는 DB에 직접 쓰지 않는다. 현재 run 실행 경로에서 Runner는 고정된 `ScenarioPlan` executor이며, Discovery 추천을 authoring하거나 ScenarioPlan을 생성/수정하지 않는다. Runner Agent Runtime은 이 경로를 바꾸지 않고 별도 `AgentTask` / `AgentTrace` / `agent.execute.request` 경로로 추가하는 후속 계획이다.
...
- `agent.execute.request`: Spring → Node Runner Agent Runtime, contract-first 후속 구현
...
Agent Runtime 후속 실행 흐름은 기존 Run materialization 흐름과 분리한다.

```text
1. Spring creates an AgentTask and publishes agent.execute.request
