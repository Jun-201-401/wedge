...
Runner는 DB에 직접 쓰지 않는다. Runner는 고정된 `ScenarioPlan` executor이며, Discovery 추천을 authoring하거나 ScenarioPlan을 생성/수정하지 않는다.
...
- Runner는 ScenarioAuthoring에 참여하지 않는다. Runner는 Spring이 materialize한 고정 `ScenarioPlan`만 실행한다.
