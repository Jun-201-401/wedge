...
Run execution message는 Spring이 고정한 `scenarioTemplateVersionId`와 materialized `scenarioPlan`을 포함한다. MVP Analyzer message는 FastAPI Analyzer 합의에 맞춰 full `evidencePacket`을 포함한다. 장기 구조에서는 저장된 packet snapshot을 가리키는 `evidencePacketId` 방식으로 되돌릴 수 있다.
...
analysis.request           # MVP payload: full evidencePacket + analysisType(PRIMARY/REPROCESS/COMPARE)
