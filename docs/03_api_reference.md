# 03. API Reference and Transport Contracts
...
Runner는 checkpoint batch를 Spring에 전달한다. Internal runner callbacks use `stepKey`, the stable ScenarioPlan `step_id`; Spring resolves it to the DB `test_run_step.id` UUID.
...
V1 optional execute/export tools, enabled only when client policy and OAuth scopes allow them:
...
Project-level access is verified in Spring DB, not only by JWT claims.
