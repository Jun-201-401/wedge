...
- Runner는 별도 `scenario-authoring.execute.request` worker에서 authoring input을 받아 candidate를 생성할 수 있다.
- Scripted Run 경로에서 Runner는 고정된 ScenarioPlan executor로 유지한다. authoring job/result를 실행 중 재해석하거나 수정하지 않는다.
- 제품 기본 Run 시작은 Agent 경로(`/api/runs/{runId}/start` → `agent.execute.request`)다. 제품 UI의 Agent Run은 executable `scenarioPlan`을 `POST /api/runs`에 싣지 않는다. 고정 ScenarioPlan replay는 명시적 scripted 경로(`/api/runs/{runId}/scripted/start` → `run.execute.request`)에서 다룬다.
- Runner Agent Runtime은 ScenarioAuthoring과 별도 경로다. Agent Runtime 작업은 `Runner Agent Runtime 구현 계획` 섹션의 문서 세트를 사용한다.
