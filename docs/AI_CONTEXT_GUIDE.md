...
- 현재 scripted Runner는 고정된 ScenarioPlan executor로 유지한다. Runner Agent 전환 작업은 `docs/wedge_runner_agent_execution.md`를 기준으로 별도 agent mode를 추가하고, ScenarioAuthoring result를 직접 실행/수정하는 구조로 섞지 않는다.
